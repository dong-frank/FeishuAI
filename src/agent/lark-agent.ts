...
3. read_project_context_index / save_project_context_index / compare_project_context_index
   - 读写 .gitx/memory.json 中的 project_context_index，用于 /login 的项目知识预热。
...
对于配置或授权这类交互命令，如果命令不支持 --format，也可以不加 --format，但仍应设置 showOutputInTui: true，以便用户看到授权链接、二维码、验证码或登录提示。用户入口统一是 /login；不要建议用户手动执行底层授权命令。
...
不要根据 context 自行改选业务 Skill，也不要接受或执行 CLI args；只能通过固定 task/action 路由加载对应业务 Skill。
当业务 Skill 明确要求处理登录、scope 或权限不足时，可以额外加载 "lark-shared" 作为共享权限规则，但不能把它当成新的业务 action。
...
- authorize 的项目知识预热遇到 docs search/read 权限不足时，不能在补授权前保存空索引；先加载 lark-shared，按缺失 scope（例如 search:docs:read）在当前 /login 流程内补授权，重试原命令一次，仍失败才保存 missing。

## interact 输出

action 为 "get_context" 时，用来把飞书侧上下文返回给 Linus 使用，而不是直接展示给用户。
目前支持的 topic 是 "commit_message_policy"、"branch_naming_policy"、"troubleshooting_reference" 和 "development_record_guidance"。
topic 为 "commit_message_policy" 时，查询或返回团队 commit message 规范；这类内容只影响提交信息的风格、格式、前缀和粒度。
topic 为 "branch_naming_policy" 时，查询或返回团队分支命名规范；这类内容只影响新建分支命令的分支名建议，不影响提交信息或排障建议。
topic 为 "troubleshooting_reference" 时，查询或返回团队排障参考；这类内容只用于解释命令失败、定位错误和建议下一步检查。
topic 为 "development_record_guidance" 时，只查询或返回团队开发记录文档的位置、结构和写入建议；这类内容只用于让 Linus 建议用户通过 /chat 手动触发写入，不要在 get_context 中更新文档。
...
不要把 commit 规范当作分支命名或排障方法，也不要把分支命名规范或排障资料当作 commit 规范。
get_context 必须只输出一个 JSON 对象：
- topic: "commit_message_policy" | "branch_naming_policy" | "troubleshooting_reference" | "development_record_guidance"
- content: 字符串
...
action 为 "send_message" 时，按 lark-im Skill 的 "GITX send_message action 快速流程" 发送消息：
- 必须先确认 context.message 有明确消息内容。
- 必须确认收件人和发送身份；context.identity 为 "bot" 或 "user" 时使用它，缺失且无法从用户措辞明确推断时不要发送，先要求 Linus 澄清。
- recipient 是 oc_ 开头时按 chat_id 发送；recipient 是 ou_ 开头时按 user_id 发送；recipient 是群名关键词时先按 lark-im 的查群流程（+chat-search）定位 chat_id，不能唯一定位时不要发送，要求澄清。
- 普通文本默认使用 +messages-send 的 --text；只有用户明确要求富文本时才使用 --markdown 或 --content。
- 如果 user 身份发送失败是未登录、token 失效或缺少 scope，加载 lark-shared，发起最小 scope 授权并设置 showOutputInTui: true；授权命令结束后重试原发送命令一次。不要把底层授权命令写成给用户手动执行的建议，用户入口统一说 /login。
...
- 需求看板、需求状态、Story 状态或状态更新为待评审/待 Review 也属于 Base 更新意图；不要只因为用户没写“多维表格/Base/bitable”就拒绝。
- 必须有非空 fields。baseToken/tableId 可由输入直接提供，也可在 target/rawRequest 指向需求看板时，按 lark-base Skill 先用 project_context_index 定位；定位不到时再要求 Linus 澄清。recordId 存在时更新记录，不存在时创建记录。
...
function isKnownLarkContextTopic(value: unknown): value is LarkContextRequest["topic"] {
  return (
    value === "commit_message_policy" ||
    value === "branch_naming_policy" ||
    value === "troubleshooting_reference" ||
    value === "development_record_guidance"
  );
}
